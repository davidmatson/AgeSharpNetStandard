...
namespace Age
{
    public static class AgeKeygen
    {
        ...
        public static IRecipient ParseSshRecipient(string authorizedKeysLine)
        {
            ...
            {
                case "ssh-ed25519":
                    return SshEd25519Recipient.Parse(authorizedKeysLine);
                case "ssh-rsa":
                    return SshRsaRecipient.Parse(authorizedKeysLine);
                default:
        ...
        public static IIdentity ParseSshIdentity(string pemText)
        {
            ...
            {
                case "ssh-ed25519":
                    return SshEd25519Identity.Parse(pemText);
                case "ssh-rsa":
                    return SshRsaIdentity.Parse(pemText);
                default:
                    throw new FormatException($"unsupported SSH key type: {keyType}");
            }
            ;
        ...
        private static IRecipient ParseRecipientLine(string line, IPluginCallbacks callbacks)
        {
            if (line.StartsWith("age1pq"))
                return MlKem768X25519Recipient.Parse(line);
            else if (line.StartsWith("age1") && line.IndexOf('1', 4) > 0)
                return new PluginRecipient(line, callbacks);
            else if (line.StartsWith("age1"))
                return X25519Recipient.Parse(line);
            else if (line.StartsWith("ssh-"))
                return ParseSshRecipient(line);
            else
                throw new FormatException($"unrecognized line in recipients file: {line}");
        }
        ...
        public static IReadOnlyList<IIdentity> DecryptIdentityFile(byte[] data, string passphrase)
        {
            ...
            using (var output = new MemoryStream())
            {
                ...
                var plaintext = EncodingExtended.UTF8.GetString(output.ToArray());
        ...
        {
            var plaintextBytes = EncodingExtended.UTF8.GetBytes(identityFileText);
