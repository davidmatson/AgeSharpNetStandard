...
namespace Age.Format
{
    internal static class AsciiArmor
    {
        ...
        public static void Armor(Stream input, Stream output)
        {
            ...
            var writer = new StreamWriter(output, EncodingExtended.UTF8, BufferSize.Default, leaveOpen: true) { NewLine = "\n" };
            ...
            while (true)
            {
                ...
                ConvertExtended.TryToBase64Chars(readBuffer.AsSpan(0, bytesRead), charBuffer, out var charsWritten);
                writer.Write(charBuffer.Slice(0, charsWritten).ToString());
                writer.WriteLine();
            }
