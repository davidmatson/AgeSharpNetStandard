...
namespace Age.Format
{
    ...
    internal sealed class DearmorStream : Stream
    {
        ...
        public int Read(Span<byte> buffer)
        {
            ...
            while (totalRead < buffer.Length)
            {
                if (_decodeCount > 0)
                {
                    ...
                    _decodeBuffer.AsSpan(_decodeOffset, toCopy).CopyTo(buffer.Slice(totalRead));
                    _decodeOffset += toCopy;
                    _decodeCount -= toCopy;
                    totalRead += toCopy;
                    continue;
        ...
        private bool DecodeNextLine()
        {
            ...
            if (!ConvertExtended.TryFromBase64Chars(line.AsSpan(), _decodeBuffer, out var bytesWritten))
        ...
        private static void ValidateCanonicalPadding(ReadOnlySpan<char> line)
        {
            ...
            var lastDataChar = line[line.Length - (padCount + 1)];
        ...
        private static int Base64Value(char c)
        {
            if (c >= 'A' && c <= 'Z')
                ...
                throw new AgeArmorException($"invalid base64 character: '{c}'");
        }
