...
namespace Age.Crypto
{
    internal static class SshKeyParser
    {
        ...
        {
            var parts = authorizedKeysLine.Trim().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length < 2)
        ...
        public static (string keyType, byte[] publicWireBytes, AsymmetricKeyParameter privateKey) ParsePrivateKey(string pemText)
        {
            ...
            else
            {
                ...
                AsymmetricCipherKeyPair kp = obj as AsymmetricCipherKeyPair;
                AsymmetricKeyParameter akp = obj as AsymmetricKeyParameter;

                if (kp != null)
                    privateKey = kp.Private;
                else if (akp != null && akp.IsPrivate)
                    privateKey = akp;
                else
                    throw new FormatException("PEM does not contain a private key");
            }
        ...
        {
            var hash = SHA256Extended.HashData(wireBytes);
