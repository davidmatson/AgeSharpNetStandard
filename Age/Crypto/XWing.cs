...
namespace Age.Crypto
{
    internal static class XWing
    {
        ...
        private static readonly byte[] XWingLabel = EncodingExtended.UTF8.GetBytes(@"\.//^\");
        ...
        public static (byte[] SharedSecret, byte[] Enc) Encaps(byte[] publicKey)
        {
            ...
            var pkM = publicKey.AsSpan().Slice(0, MlKemPublicKeySize);
            var pkX = publicKey.AsSpan().Slice(MlKemPublicKeySize);

            // ML-KEM-768 encapsulate
            var mlKemPub = MLKemPublicKeyParameters.FromEncoding(MLKemParameters.ml_kem_768, pkM.ToArray());
            ...
            agreement.Init(ekX);
            agreement.CalculateAgreement(new X25519PublicKeyParameters(pkX.ToArray()), ssX, 0);
            ...
            var sharedSecret = CombineSharedSecret(ssM, ssX, ctX, pkX.ToArray());
        ...
        public static byte[] Decaps(byte[] enc, byte[] seed)
        {
            ...
            var ctM = enc.AsSpan().Slice(0, MlKemCiphertextSize);
            var ctX = enc.AsSpan().Slice(MlKemCiphertextSize);

            // ML-KEM-768 decapsulate
            var decapsulator = new MLKemDecapsulator(MLKemParameters.ml_kem_768);
            decapsulator.Init(mlKemPrivate);
            var ssM = new byte[SharedSecretSize];
            decapsulator.Decapsulate(ctM.ToArray(), 0, MlKemCiphertextSize, ssM, 0, SharedSecretSize);
            ...
            try
            {
                agreement.CalculateAgreement(new X25519PublicKeyParameters(ctX.ToArray()), ssX, 0);
            }
            ...
            return ssX.All(b => b == 0)
                ...
                : CombineSharedSecret(ssM, ssX, ctX.ToArray(), pkX);
        }
