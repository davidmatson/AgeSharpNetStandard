...
namespace Age.Crypto
{
    ...
    internal static class Bech32
    {
        ...
        private static readonly int[] Generator =
            new[] { 0x3b6a57b2, 0x26508e6d, 0x1ea119fa, 0x3d4233dd, 0x2a1462b3 };
        ...
        public static (string Hrp, byte[] Data) Decode(string bech)
        {
            ...
            var hrp = lower.AsSpan().Slice(0, sepPos);
            var dataStr = lower.AsSpan().Slice(sepPos + 1);
            ...
            if (!VerifyChecksum(hrp.ToString(), data5))
                throw new FormatException("invalid bech32 checksum");

            // Strip checksum
            var data5NoCheck = data5.AsSpan().Slice(0, data5.Length - 6);
            var data8 = ConvertBits(data5NoCheck, 5, 8, false);

            return (hrp.ToString(), data8);
        }
