...
namespace Age.Recipients
{
    public sealed class ScryptRecipient : IRecipient, IIdentity
    {
        ...
        private static int EnsureValidWorkFactor(int workFactor) =>
            workFactor >= 1 && workFactor <= MaxWorkFactor
                ? workFactor
        ...
        public Stanza Wrap(ReadOnlySpan<byte> fileKey)
        {
            ...
            RandomNumberGeneratorExtended.Fill(salt);
            ...
            return new Stanza(StanzaType, new[] { saltB64, _workFactor.ToString() }, body);
        }
        ...
        public byte[] Unwrap(Stanza stanza)
        {
            ...
            try
            {
                salt = Base64Unpadded.Decode(stanza.Args[0].AsSpan());
            }
        ...
        private static byte[] DeriveWrapKey(string passphrase, byte[] salt, int workFactor)
        {
            ...
            var scryptSalt = ArrayFactory.Concatenate(labelBytes, salt);

            var n = 1 << workFactor;
            var passphraseBytes = EncodingExtended.UTF8.GetBytes(passphrase);
