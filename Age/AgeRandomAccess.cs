...
namespace Age
{
    public sealed class AgeRandomAccess : IDisposable
    {
        ...
        public AgeRandomAccess(Stream ciphertext, params IIdentity[] identities) :
            this(ciphertext, identities.AsSpan())
        {
        }

        public AgeRandomAccess(Stream ciphertext, ReadOnlySpan<IIdentity> identities)
        {
            if (!ciphertext.CanSeek)
                throw new ArgumentException("ciphertext stream must be seekable", nameof(ciphertext));

            _binaryStream = ciphertext;
        ...
        public int ReadAt(long plaintextOffset, Span<byte> buffer)
        {
            ...
            while (totalRead < buffer.Length && currentOffset < PlaintextLength)
            {
                ...
                plaintext.AsSpan(offsetInChunk, toCopy).CopyTo(buffer.Slice(totalRead));
        ...
        private Stream BinaryStream =>
            _armoredBinaryInput ?? _binaryStream;

        private readonly struct PayloadInfo
        {
            readonly byte[] _payloadKey;
            readonly long _payloadStart;
            readonly long _totalEncrypted;
            readonly long _plaintextLength;
                ...
                byte[] PayloadKey, long PayloadStart, long TotalEncrypted, long PlaintextLength)
            {
                ...
                _totalEncrypted = TotalEncrypted;
                _plaintextLength = PlaintextLength;
            }

            public byte[] PayloadKey => _payloadKey;
            public long PayloadStart => _payloadStart;
            public long TotalEncrypted => _totalEncrypted;
