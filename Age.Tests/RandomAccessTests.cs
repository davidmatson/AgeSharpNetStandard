...
namespace Age.Tests
{
    public class RandomAccessTests
    {
        ...
        public void FirstByte()
        {
            using (var identity = X25519Identity.Generate())
            {
                var plaintext = EncodingExtended.UTF8.GetBytes("Hello, random access!");
        ...
        public void ReadPastEnd_ReturnsZero()
        {
            using (var identity = X25519Identity.Generate())
            {
                var plaintext = EncodingExtended.UTF8.GetBytes("short");
        ...
        public void SingleChunk_ExactSize()
        {
            using (var identity = X25519Identity.Generate())
            {
                ...
                using (var ra = new AgeRandomAccess(ciphertext, identity))
                {
                    ...
                    int read = ra.ReadAt(plaintext.Length - 1, buf);
                    Assert.Equal(1, read);
                    Assert.Equal(plaintext[plaintext.Length - 1], buf[0]);
                }
            }
        }
        ...
        public void Armored_RandomAccess()
        {
            using (var identity = X25519Identity.Generate())
            {
                var plaintext = EncodingExtended.UTF8.GetBytes("armored random access test data!");
