...
namespace Age.Tests
{
    public class Base64UnpaddedTests
    {
        ...
        public void Encode_Decode_RoundTrip(int length)
        {
            ...
            var decoded = Base64Unpadded.Decode(encoded.AsSpan());
        ...
        {
            var ex = Assert.Throws<FormatException>(() => Base64Unpadded.Decode("AAAA==".AsSpan()));
        ...
        public void NonCanonical_Encoding_Rejected()
        {
            ...
            var ex = Assert.Throws<FormatException>(() => Base64Unpadded.Decode("AB".AsSpan()));
        ...
        {
            Assert.Throws<FormatException>(() => Base64Unpadded.Decode("@@@@".AsSpan()));
        }
    ...
    public class Bech32Tests
    {
        ...
        public void Mixed_Case_Rejected()
        {
            using (var identity = X25519Identity.Generate())
            {
                var recipientStr = identity.Recipient.ToString();
                // Force mixed case
                var mixed = recipientStr.AsSpan().Slice(0, 4).ToString() + recipientStr.AsSpan().Slice(4).ToString().ToUpperInvariant();
        ...
        public void Invalid_Checksum_Rejected()
        {
            using (var identity = X25519Identity.Generate())
            {
                ...
                chars[chars.Length - 1] = chars[chars.Length - 1] == 'q' ? 'p' : 'q';
    ...
    public class HeaderReaderTests
    {
        ...
        {
            var stream = new MemoryStream(EncodingExtended.UTF8.GetBytes("hello\nworld\n"));
        ...
        {
            var stream = new MemoryStream(EncodingExtended.UTF8.GetBytes("line2\n"));
            var reader = new HeaderReader(stream);
        ...
        {
            var content = EncodingExtended.UTF8.GetBytes("abc\ndef\n");
    ...
    public class StanzaTests
    {
        ...
        public void Parse_Valid_Stanza_With_Args_And_Body()
        {
            ...
            Assert.Equal(EncodingExtended.UTF8.GetBytes("test"), stanza.Body.ToArray());
        }
    ...
    public class HeaderTests
    {
        ...
        public void ComputeMac_Deterministic()
        {
            ...
            var headerBytes = EncodingExtended.UTF8.GetBytes("age-encryption.org/v1\n-> test\n\n---");
    ...
    public class AsciiArmorTests
    {
        ...
        public void Reject_Trailing_NonWhitespace_Data()
        {
            ...
            using (var armored = new MemoryStream())
            {
                ...
                armored.Write(EncodingExtended.UTF8.GetBytes("extra"));
                armored.Position = 0;
        ...
        public void Allows_Trailing_Whitespace_After_End()
        {
            ...
            using (var armored = new MemoryStream())
            {
                ...
                armored.Write(EncodingExtended.UTF8.GetBytes("\n  \n"));
                armored.Position = 0;
    ...
    public class StreamEncryptionTests
    {
        ...
        {
            var payloadKey = MakePayloadKey();
            var plaintext = EncodingExtended.UTF8.GetBytes("test data");
    ...
    public class AgeEncryptTests
    {
        ...
        public void Encrypt_Rejects_No_Recipients()
        {
            ...
            using (var output = new MemoryStream())
            {
                Assert.Throws<ArgumentException>(() => AgeEncrypt.Encrypt(input, output, Array.Empty<IRecipient>()));
            }
        }
        ...
        public void Armored_Encrypt_Decrypt_RoundTrip()
        {
            using (var identity = X25519Identity.Generate())
            {
                var plaintext = EncodingExtended.UTF8.GetBytes("armored test");
        ...
        public void Decrypt_Rejects_No_Matching_Identity()
        {
            ...
            using (var id2 = X25519Identity.Generate())
            {
                var plaintext = EncodingExtended.UTF8.GetBytes("test");
    ...
    public class DecryptStreamTests
    {
        ...
        public void DecryptReader_Rejects_Truncated_Payload_NoFinalChunk()
        {
            using (var identity = X25519Identity.Generate())
            {
                ...
                using (var encOutput = new MemoryStream())
                {
                    ...
                    var truncated = ciphertextBytes.AsSpan().Slice(0, truncateAt);

                    using (var truncatedStream = new MemoryStream(truncated.ToArray()))
        ...
        public void DecryptReader_Rejects_Chunk_TooSmallForTag()
        {
            using (var identity = X25519Identity.Generate())
            {
                var plaintext = EncodingExtended.UTF8.GetBytes("small payload");
