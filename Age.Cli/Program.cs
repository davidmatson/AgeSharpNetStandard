...
class Program
{
    public static int Main(string[] args)
    {
        ...
        rootCommand.SetAction(parseResult =>
            AgeCommand.Execute(
                ...
                parseResult.GetValue(recipientOption) ?? Array.Empty<string>(),
                parseResult.GetValue(recipientsFileOption) ?? Array.Empty<string>(),
                parseResult.GetValue(identityOption) ?? Array.Empty<string>(),
        ...
        catch (Exception ex) when (ex is AgeException || ex is FormatException)
        {
    ...
    {
        switch (parsed.CommandResult.Command.Name)
        {
            case "keygen":
                return "age-keygen";
            case "inspect":
